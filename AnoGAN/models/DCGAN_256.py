...
class Generator(nn.Module):
    def __init__(self, z_dim, c_dim, gf_dim, ksize):
        ...
        self.layer4 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(gf_dim >> 1, gf_dim >> 2, ksize, 2, 1, bias=False),
            nn.BatchNorm2d(gf_dim >> 2),
            nn.ReLU(inplace=True),
        )
        self.layer5 = nn.Sequential(
            nn.ConvTranspose2d(gf_dim >> 2, c_dim, ksize, 2, 1, bias=False),
            nn.Tanh(),
        )
...
class Discriminator(nn.Module):
    def __init__(self, c_dim, df_dim):
        ...
        self.layer4 = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(0.5),
            nn.Conv2d(df_dim*16, df_dim*32, 4, 2, 1, bias=False),
            nn.BatchNorm2d(df_dim*32),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(0.5),
        )
        self.layer5 = nn.Sequential(
            nn.Conv2d(df_dim*32, 1, 4, 1, 0, bias=False),
            nn.Sigmoid(),
        )
