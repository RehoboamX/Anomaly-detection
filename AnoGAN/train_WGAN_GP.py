...
from models.WGAN_GP import Generator, Discriminator, initialize_weights, gradient_penalty
...
with tqdm.tqdm(range(opt.n_epoches)) as t_epoches:
    for epoch in t_epoches:
        ...
        for idx, inputs in enumerate(train_dataloader):
            ...
            gp = gradient_penalty(disc, inputs, outputs, device=device)

            disc_loss = - (torch.mean(D_real) - torch.mean(D_fake)) + opt.lambda_gp * gp
